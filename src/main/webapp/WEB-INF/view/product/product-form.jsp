        ...
        <tr>
            ...
            <td><input type="submit" value="บันทึก" class="save"/>
                <input type="button" value="ลบ"
                       onclick="if((confirm('คุณแน่ใจหรือว่าต้องการลบสินค้านี้?'))) { window.location.href='${pageContext.request.contextPath}/product/${product.id}/delete';
                           return false; }"
                       class="cancel-button"/>
                <c:if test="${product.id > 0}"><input type="button" value="แสดงร้านค้า"
                                                      onclick="window.location.href='${pageContext.request.contextPath}/product/${product.id}/view-shops';
                                                      return false;"class="add-button"/></c:if>
